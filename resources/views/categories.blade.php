@extends('layouts.main')
@section('container')
<article class="mb-5">
    <h1 class="mb-5">Halaman Post Categories</h1>

    <div class="container">
        <div class="row">
            @foreach ($categories as $category)
            <div class="col-md 4">
                <a href="/shop?category={{ $category->slug }}">
                <div class="card bg-dark text-white">
                    <img src="https://source.unsplash.com/1200x400?{{ $category->name }}" class="card-img" alt="{{ $category->name }}">
                    <div class="card-img-overlay d-flex alig-items-center p-0">
                      <h5 class="card-title text-center flex-fill p-4" style="background-color: rgba(0, 0, 0, 0.7) fs-3">{{ $category->name }}</h5>
                    </div>
                </div>
                </a>
            </div>
            @endforeach
        </div>
    </div>
</article>
@endsection